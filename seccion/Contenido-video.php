...
$desde=($_GET['p'] * $hasta) - $hasta ;
 		...
 		$consultar="SELECT count(id) FROM contenido WHERE $SQLTIPO AND activo='1' and video!='' and  (id='0' $SQL)  ";
...
$consultas="SELECT * FROM contenido WHERE $SQLTIPO AND activo='1' and video!='' and  (id='0' $SQL) ORDER BY tipo, orden , fecha DESC, act DESC LIMIT ".$desde.", ".$hasta.";; ";
...
$sql=mysqli_query($conex,$consultas);
	...
	while ($contenido=mysqli_fetch_array($sql)){
		$a++;$b++;
 ?>
	 <div class="item white" data-w="800" data-h="600"> <a href="/<? echo $contenido['slug']; ?>"> <img class="loading" src="https://img.youtube.com/vi/<? echo $contenido['video']; ?>/hqdefault.jpg" alt="<? echo $contenido['titulo']; ?>" title="<? echo $contenido['titulo']; ?>" onerror="this.src.style.display='none'"> </a> <div> <a href="/<? echo $contenido['slug']; ?>"><i class="fa fa-play"></i> <? echo $contenido['titulo']; ?></a> </div> </div>
